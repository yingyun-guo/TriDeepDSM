...
class TriViewDataset(Dataset):
    def __init__(self, mode='training', config=None):
        ...
        if config and 'PATH_META' in config:
            ...
            self.root_meta = os.path.join(PROJECT_ROOT, "Datasets", "processed_meta")
        self.root_shape_npy = os.path.join(PROJECT_ROOT, "Datasets", "processed_shape")
        self.root_onehot = os.path.join(PROJECT_ROOT, "feature", "one-hot")
        ...
        if mode in ['training', 'testing']:
            ...
            self.vcf_path = os.path.join(PROJECT_ROOT, "data", f"{mode}_hg19_final.vcf")
