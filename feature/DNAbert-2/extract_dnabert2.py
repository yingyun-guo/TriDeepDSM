...
# os.environ["USE_TORCH"] = "1"
#
# # 参考基因组路径 (必须存在且已建立索引，通常要有 .fai 文件)
# REF_GENOME = "/data/gyy/Data/hg19.fa/hg19.fa"
#
# # 输入 TXT 文件路径
# INPUTS = {
...
# def get_sequences_from_txt(txt_path, ref_fasta_path):
#     """
#     读取 txt 文件，根据 chr 和 pos 从参考基因组提取序列
#     """
#     print(f"Loading coordinates from {txt_path}...")
#
#     # 读取数据，使用 tab 分隔
...
#         pos = int(row['pos'])
#
#         # 处理染色体命名不一致问题 (比如 txt 是 "1", hg19 是 "chr1")
#         if not chrom.startswith("chr"):
#             chrom = f"chr{chrom}"
#
#         # 处理特殊的 MT / M
...
#         # 检查染色体是否存在于 reference 中
#         if chrom not in fasta.references:
#             # 尝试不带 chr 的情况，防止 reference 本身就没有 chr 前缀
#             if chrom.replace("chr", "") in fasta.references:
#                 chrom = chrom.replace("chr", "")
#             else:
#                 # print(f"Warning: {chrom} not found in reference.") # 调试时可开启
#                 continue
#
#         # 计算提取范围 (hg19 pos 是 1-based, pysam fetch 是 0-based)
#         # 目标: 以 pos 为中心，提取 radius 长度
#         # pos (1-based) -> pos-1 (0-based)
#         start = (pos - 1) - RADIUS
#         end = (pos - 1) + RADIUS
#
#         # 边界检查
...
#             seq = fasta.fetch(chrom, start, end).upper()
#
#             # 简单检查长度，如果不够（比如在染色体末端），可能需要 padding，这里简单跳过或补N
#             if len(seq) < (RADIUS * 2):
#                 seq = seq.ljust(RADIUS * 2, 'N')
#
#             seqs.append(seq)
...
#             batch = seqs[i: i + BATCH_SIZE]
#
#             # Tokenize: 这里的 max_length 对应提取的序列长度
#             inputs = tokenizer(batch, return_tensors="pt", padding="max_length", max_length=150, truncation=True).to(
#                 DEVICE)
#
#             with torch.no_grad():
...
#         final_feat = np.concatenate(feats, axis=0)
#
#         # 保存文件名: MFDSMC_test1_dnabert2.npy
#         save_path = os.path.join(SAVE_DIR, f"{name}_dnabert2.npy")
#         np.save(save_path, final_feat)
#         print(f"  Saved to: {save_path} | Shape: {final_feat.shape}")
#
